...
Ventana: rolling VENTANA_HORAS horas
...
VENTANA_HORAS = int(os.getenv("VENTANA_HORAS", "3"))
...
COORD_MAP = {
    ...
    "cuernavaca acapulco": (18.20, -99.20), "amozoc": (19.10, -98.00),
    "tinaja isla": (18.10, -95.20), "arco norte": (19.80, -99.10),
    ...
    "leon silao": (21.12, -101.68), "celaya queretaro": (20.55, -100.60),
    ...
    "culiacan mazatlan": (23.70, -106.70), "tijuana ensenada": (31.70, -116.70),
    "mexicali tijuana": (32.20, -115.50), "navojoa hermosillo": (28.00, -109.90),
    "chihuahua ciudad juarez": (30.40, -106.40), "durango mazatlan": (24.50, -106.00),
    "veracruz coatzacoalcos": (18.20, -94.80), "oaxaca istmo": (16.50, -95.00),
    "cordoba veracruz": (18.80, -96.90), "xalapa veracruz": (19.35, -96.60),
    ...
    "chihuahua": (28.64, -106.08), "baja california": (30.84, -115.28),
    "baja sur": (23.70, -110.00), "coahuila": (27.06, -101.71),
    "durango": (24.02, -104.66), "zacatecas": (22.77, -102.58),
    ...
    "aguascalientes": (21.88, -102.29), "tabasco": (17.99, -92.93),
    "campeche": (19.83, -90.53), "yucatan": (20.97, -89.62), "quintana roo": (18.50, -88.30),
    ...
    "acapulco": (16.86, -99.88), "cancun": (21.16, -86.85), "merida": (20.97, -89.62),
    "cuernavaca": (18.92, -99.23), "toluca": (19.29, -99.66), "pachuca": (20.12, -98.73),
    "xalapa": (19.53, -96.91), "villahermosa": (17.99, -92.93), "tuxtla": (16.75, -93.12),
}
...
def esta_en_ventana(rss_date):
    """Solo acepta articulos de las ultimas VENTANA_HORAS horas (ventana rodante).
...
def fetch_google_news():
    ...
    for query in GNEWS_QUERIES:
        ...
        for e in feed.entries[:MAX_POR_FEED]:
            pub = e.get("published", "")
            if not esta_en_ventana(pub):
                continue
            titulo  = limpiar(e.get("title", ""))
            resumen = limpiar(e.get("summary", ""))
            ...
            alertas.append(hacer_alerta(
                ...
                e.get("link", ""), texto, pub,
            ))
            n += 1
        log.info(f"  GNews '{query[:40]}': {len(feed.entries)} -> {n}")
        time.sleep(0.4)
    log.info(f"  Google News total: {len(alertas)}")
    return alertas
    ...
    for nombre, rss_url in feeds:
        ...
        for e in feed.entries[:MAX_POR_FEED]:
            ...
            alertas.append(hacer_alerta(
                clasificar(texto), extraer_ruta(titulo), texto[:500],
                extraer_rec(texto), fmt_fecha(pub),
                nombre, e.get("link", rss_url), texto, pub,
            ))
            n += 1
...
def fetch_rss_nacionales():
    ...
    log.info(f"  Nacionales: {len(r)}")
    return r


def fetch_rss_regionales():
    r = _procesar_rss(RSS_REGIONALES)
    log.info(f"  Regionales: {len(r)}")
    return r
    ...
    for url in CAPUFE_URLS:
        ...
        if "xml" in ct or url.endswith(".xml"):
            ...
            if feed.entries:
                ...
                log.info(f"  CAPUFE XML: {len(alertas)}")
                return alertas
        ...
        if alertas:
            log.info(f"  CAPUFE HTML: {len(alertas)}")
            return alertas
    log.info(f"  CAPUFE: {len(alertas)}")
    return alertas
    ...
    for url in CONAGUA_URLS:
        ...
        if "xml" in ct or url.endswith(".xml"):
            ...
            if feed.entries:
                ...
                log.info(f"  CONAGUA: {len(alertas)}")
                return alertas
    log.info(f"  CONAGUA: {len(alertas)}")
    return alertas
    ...
    for cuenta in CUENTAS_X:
        for base in NITTER_INSTANCES:
            ...
            for e in feed.entries[:30]:
                ...
                n += 1
            log.info(f"  Nitter @{cuenta} via {base}: {n}")
            break
    log.info(f"  Nitter total: {len(alertas)}")
    return alertas
...
def dedup(alertas):
    ...
    for a in alertas:
        ...
        if any(len(tok) > 3 and len(tok & st) / max(len(tok), 1) >= 0.80 for st in seen_tok):
            continue
        seen_ids.add(uid)
        seen_tok.append(tok)
        out.append(a)
...
def filtrar_resueltos(alertas):
    ...
    for a in alertas:
        if es_resuelto(a["descripcion"]):
            ...
            if llave:
                resueltos.add(llave)
                log.info(f"  Tramo liberado: {llave}")
    if not resueltos:
        return alertas
    out = []
    for a in alertas:
        if es_resuelto(a["descripcion"]):
            continue
        llave = _llave(a["descripcion"] + " " + a["ruta"])
        if llave and llave in resueltos:
            log.info(f"  Removida (liberado): {a['ruta'][:50]}")
            continue
        out.append(a)
    return out
...
# ── Main ──────────────────────────────────────────────────────────────
def main():
    log.info(f"=== Scraper Alertas Viales Mexico v2 | modo={RUN_MODE} ventana={VENTANA_HORAS}h ===")
    todas, fuentes = [], []
    ...
    if RUN_MODE in ("all", "media_only"):
        ...
        try:
            r = fetch_rss_nacionales()
            todas.extend(r)
            if r:
                fuentes.append("Medios nacionales")
        except Exception as e:
            log.error(f"  Nacionales: {e}")

        log.info(">> RSS regionales...")
        try:
            r = fetch_rss_regionales()
            todas.extend(r)
            if r:
                fuentes.append("Medios regionales")
        except Exception as e:
    ...
    resultado = {
        ...
        "ventana":   f"ultimas {VENTANA_HORAS} horas",
        "total":     len(todas),
