...
# ─────────────────────────────────────────────────────────────────────
LOOKBACK_HORAS = int(os.getenv("LOOKBACK_HORAS", "24"))   # ventana de tiempo
MAX_POR_FEED   = int(os.getenv("MAX_POR_FEED",   "30"))   # entradas a leer por feed
RUN_MODE       = os.getenv("RUN_MODE", "all")             # all | official_only | media_only
...
def esta_en_ventana(rss_date: str) -> bool:
    ...
    Si no se puede parsear la fecha se acepta (mejor mostrar de más que de menos)."""
...
def dedup(alertas: list[dict]) -> list[dict]:
    ...
    for a in alertas:
        ...
        # Rechazar si 80%+ de tokens coinciden con una alerta ya vista
        duplicado = False
        for st in seen_tokens:
            if len(tok) > 3 and len(tok & st) / max(len(tok), 1) >= 0.80:
                duplicado = True
                break
        if duplicado:
            continue
...
def main():
    ...
    # Sin alertas → conservar JSON anterior
    if not todas:
